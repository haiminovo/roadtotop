Road To Top currently ships a Day0 idle-MMO vertical slice built on the existing single-package setup:
...
- legacy websocket prototype still present in `server/`, but the Day0 gameplay loop now uses REST APIs
...
Initialize the Day0 schema and item seed data:
...
The Day0 frontend uses these routes:
